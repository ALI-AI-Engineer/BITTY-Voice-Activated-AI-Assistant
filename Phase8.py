...
graph.add_conditional_edges(
    ...
    route_decision,
    {
        ...
        "to_plan": "planner",  # fix: properly map "to_plan"
        "to_critic": "self_critic",
    }
)

# Loops for tools and MCP
...
# Error handler → router (retry after handling)
graph.add_edge("error_handler", "llm_router")

# # Final answer → critic → end
# graph.add_edge("final_answer", "self_critic")
graph.add_edge("llm_router", "self_critic")
graph.add_conditional_edges(
    "self_critic",
    self_critic_router,
    {
        "retry": "llm_router",   # go back for refinement
        "accept": "final_answer"          # finish
    }
)
